...
const DOLLAR_FIGI = 'BBG0013HGFT4';
...
void main(List<String> arguments) {
  ...
  Jaguar(port: port)
    ..getJson('/portfolio', getPortfolio, before: [cors(options)])
    ..serve();
...
Future<num> calcPositionSumInRub(
  ...
  for (final position in positions) {
    final lastPrice = await getLastPrice(position.figi);
...
num calcCurrencyPositionSumInRub(
...
Future<num> getLastPrice(String figi) async {
...
FutureOr<dynamic> getPortfolio(Context ctx) async {
  ...
  final stocksPositions =
      positions.where((x) => x.ticker == 'AKNX' || x.ticker == 'FXIT').toList();
